...
Library='[103m[30m'"------> Library <------"'[0m'
	...
	echo $Prueba "1"
	echo >> times_c_getime.txt
	echo $Prueba "1" >> times_c_getime.txt
	echo >> times_c_getime.txt
	sudo \perf stat -a --output times_c_getime_perf.txt --append -r 5 ./$Prefijo"c_getime" "5" "false" "false" >> matrices_c_getime_perf.csv
	...
	for i in $(seq 1 $inferior_limite); do
		...
		echo $Prueba $j
		echo >> times_c_getime.txt
		echo $Prueba $j >> times_c_getime.txt
		echo >> times_c_getime.txt
		sudo \perf stat -a --output times_c_getime_perf.txt --append -r 5 ./$Prefijo"c_getime" $k "false" "false" >> matrices_c_getime_perf.csv
