...
Library='[103m[30m'"------> Library <------"'[0m'
	...
	echo $Prueba "1"
	\time -o times_c_clock.txt -a ./$Prefijo"c_clock" "5" "false" "false" >> matrices_c_clock.csv
	# Testing matrices of dimension 10,20,30,..., 100
	for i in $(seq 1 $inferior_limite); do
		let j=($i + 1)
		let k=($i * 10)
		echo $Prueba $j
		\time -o times_c_clock.txt -a ./$Prefijo"c_clock" $k "false" "false" >> matrices_c_clock.csv
