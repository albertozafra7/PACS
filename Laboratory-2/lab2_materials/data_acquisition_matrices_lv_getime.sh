...
Library='[103m[30m'"------> Library <------"'[0m'
	...
	echo >> times_lv_getime.txt
	\time -o times_lv_getime.txt -a ./$Prefijo"lv_getime" "5" "false" "false" >> matrices_lv_getime.csv
	...
	for i in $(seq 1 $inferior_limite); do
		...
		echo >> times_lv_getime.txt
		\time -o times_lv_getime.txt -a ./$Prefijo"lv_getime" $k "false" "false" >> matrices_lv_getime.csv
