...
Library='[103m[30m'"------> Library <------"'[0m'
	...
	echo $Prueba "1"
	echo >> strace_c_getime.txt
	echo $Prueba "1" >> strace_c_getime.txt
	echo >> strace_c_getime.txt
	\strace -o strace_c_getime.txt -c -A ./$Prefijo"c_getime" "5" "false" "false" >> matrices_c_getime_strace.csv
	...
	for i in $(seq 1 $inferior_limite); do
		...
		echo $Prueba $j
		echo >> strace_c_getime.txt
		echo $Prueba $j >> strace_c_getime.txt
		echo >> strace_c_getime.txt
		\strace -o strace_c_getime.txt -c -A ./$Prefijo"c_getime" $k "false" "false" >> matrices_c_getime_strace.csv
