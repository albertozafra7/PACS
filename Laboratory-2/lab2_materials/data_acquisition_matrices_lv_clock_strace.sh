...
Library='[103m[30m'"------> Library <------"'[0m'
	...
	echo $Prueba "1"
	echo $Prueba "1" >> strace_lv_clock.txt
	\strace -o strace_lv_clock.txt -c -A ./$Prefijo"lv_clock" "5" "false" "false" >> matrices_lv_clock_strace.csv
	# Testing matrices of dimension 10,20,30,..., 100
	for i in $(seq 1 $inferior_limite); do
		let j=($i + 1)
		let k=($i * 10)
		echo $Prueba $j
		echo $Prueba $j >> strace_lv_clock.txt
		\strace -o strace_lv_clock.txt -c -A ./$Prefijo"lv_clock" $k "false" "false" >> matrices_lv_clock_strace.csv
