...
Library='[103m[30m'"------> Library <------"'[0m'
	...
	echo >> times_c_getime.txt
	\time -o times_c_getime.txt -a ./$Prefijo"c_getime" "5" "false" "false" >> matrices_c_getime.csv
	...
	for i in $(seq 1 $inferior_limite); do
		...
		echo >> times_c_getime.txt
		\time -o times_c_getime.txt -a ./$Prefijo"c_getime" $k "false" "false" >> matrices_c_getime.csv
